...
- Application-level field encryption is **not in force**: there is no per-field PHI encryption on the API write path, so transcripts and assessment results sit in plaintext within the KMS-encrypted Aurora volume (the back-office analysis pipeline reads them in plaintext). A dead, non-functional client utility (`PHIEncryption.ts`) was retired June 2026. Implementing it on the safe subset (transcripts and `fusion_outputs.analysis`, KMS-backed key, decrypt-on-read) is tracked as a security follow-up.
