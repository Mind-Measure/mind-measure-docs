...
- **2026-06 — Ops command centre build-out.** `mind-measure-ops` gained a dual-database cost rollup (uni + pro `cost_tracking`, per-customer breakdown, cost-per-check-in), a live "recently shipped" feed reading commits straight from the GitHub org, the investor deck in the founder data room, and a planning-board card CLI (`scripts/board.js`) plus board UI that moves cards between columns. Planning board reconciled against git ground truth (backfilled Done cards, fixed Wellbeing Protection phase placement).
- **2026-06 — University handbook brought to parity with Pro.** Rebuilt `mind-measure-docs/pages/handbook.mdx` into Part A (understanding) / Part B (step-by-step, screenshot-led walkthroughs: dashboard, Content Hub, article editor, Content Pool, nudges, cohort analysis, alerts, Insight Hub) / Part C (reference); added six university screenshots under `public/images/handbook/uni-*`. Also fixed Pro-isms in the university Insight Hub copy ("Voice of the workforce" → "Voice of the students", metadata, favicon label, question/cohort-report wording).
